    ...
    for user in user_list:
        for password in pass_list:
            ...
            for flag in flag_list:
                if flag in res_html:
                    info = '%s -U %s -P %s'%(login_url,user,password)
...
def outputLog(scanType,host,port,msg=''):
    ...
    if scanType==gSCAN:
        print u'[%s]: [%s:%d] status= [PORT OK]'%(timeStr,host,int(port))
    elif scanType==gCHECK:
        print '[%s]: [ http://%s:%d ] can be [W00t3D]'%(timeStr,host,int(port))
    elif scanType==gCRACKSTART:
        print '[%s]: [brute apache server] http://%s:%d'%(timeStr,host,int(port))
...
def threadJoin(m_count):
    ...
    while True:
        ...
        if (gQueue.empty() and threading.activeCount() <= 1) or i>8:
            print '[brute end] software:w00t3r.py'
            break
